...
export const PublicationsForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let imageUrl = null;
      if (formData.imagen) {
        const storageRef = ref(storage, `publications/images/${formData.imagen.name}`);
      ...
      let fileUrl = null;
      if (formData.archivo) {
        const storageRef = ref(storage, `publications/files/${formData.archivo.name}`);
    ...
    // Aquí puedes enviar el formulario a alguna API o Firestore
  };
